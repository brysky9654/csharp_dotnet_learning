...
namespace SharpLearning.Optimization
{
    ...
    public sealed class ParticleSwarmOptimizer : IOptimizer
    {
        ...
        private readonly int m_numberOfParticles;
        private readonly double m_c1;
        private readonly double m_c2;
        private readonly Random m_random;
        private readonly IParameterSampler m_sampler;
        private readonly int m_maxDegreeOfParallelism;
        private readonly object m_bestLocker;
        ...
        /// <summary>
        ///     Optimization using swarm optimization. Returns the first result below the threshold.
        /// </summary>
        /// <param name="functionToMinimize"></param>
            ...
            out OptimizerResult best)
        {
            ...
            var pBest = Enumerable.Range(0, m_numberOfParticles).Select(p => new double[m_parameters.Length]).ToArray();

            var pBestScores = Enumerable.Range(0, m_numberOfParticles).Select(p => double.MaxValue).ToArray();
            ...
            for (var iterations = 0; iterations < m_maxIterations; iterations++)
            {
                ...
                for (var i = 0; i < m_numberOfParticles; i++)
                {
                    ...
                    particleVelocities[i] = particleVelocities[i]
                        .Add(
                            pBest[i]
                                .Subtract(particles[i])
                                .Multiply(m_c1 * m_random.NextDouble())
        ...
        private void BoundCheck(double[] newValues, double[] maxValues, double[] minValues)
        {
